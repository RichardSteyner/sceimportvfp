	...
	View.prototype.addMonth = function() {
		...
		if (root.monthId%12 != 0 && root.showMonths) {
			if (root.monthId%root.showEveryNthMonth == 0) {
				$('.timeline-months').append('<div class="timeline-month timeline-dateblock" id="timeline-month-'+root.monthId+'">'+model.allMonths[root.monthId%12]+' '+model.startYear.toString().slice(-2)+'</div>');
			} else {
	...
	View.prototype.positionEvents = function() {
		...
		for (i=0; i<model.events.length; i++) {
            if(i==0){
                ...
                console.log('Height'+$('#timeline-event-'+model.events[i-1].id).position);
                $('#timeline-event-'+model.events[i].id).css({ "left" : model.events[i].position, "top" : i*90, "z-index" : i });
            }
		}
	},
